...
### What can we add? Smaller tasks (~4 each ... up to 48)

- (4) When the user types ``help``; print the instructions again. Add this option to the listing of instructions. You'll need Python's ``continue`` statement. [Youtube Starter Video](https://youtu.be/eLTQxvLA99k)
- If it's not nailed down...
    ...
    - (6) Locked doors should appear, even if they're not usable -- modify ``find_usable_exits`` and then modify the code that moves from place to place to detect locked exits that are not passable (given the player's current ``stuff``!) and print out a message like ``"You try to open the door, but it's locked!"``. [Youtube Starter Video](https://youtu.be/og65A2Qqgzw).
